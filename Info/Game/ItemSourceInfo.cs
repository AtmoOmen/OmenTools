...
public class ItemSourceInfo
{
    ...
    {
        if (DService.Instance().PI.TryGetData("OmenTools.Infos.ItemShopInfo.ItemToItemShopInfos", out Dictionary<uint, ItemSourceInfo> _))
        {
    ...
    public static ItemSourceInfo? GetItemInfo(uint itemID) =>
        !IsDataLoaded
            ? null
            : DService.Instance().PI.GetData<Dictionary<uint, ItemSourceInfo>>("OmenTools.Infos.ItemShopInfo.ItemToItemShopInfos")?.GetValueOrDefault(itemID);
