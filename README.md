...
OmenTools 采用 **中心化服务容器 + 自动发现服务 + 优先级任务队列** 的架构模式：

```
┌─────────────────────────────────────────┐
│              DService (容器)              │
│  ├─ Dalamud 原生服务注入 ([PluginService])│
│  ├─ OmenService 自动发现与生命周期管理     │
│  └─ 资源追踪 (TaskHelper / Hook / Patch) │
├─────────────────────────────────────────┤
│         OmenService (业务服务层)          │
│  ├─ FrameworkManager (帧更新调度)         │
│  ├─ GamePacketManager (封包拦截)          │
│  ├─ UseActionManager (技能动作事件)       │
│  ├─ ChatManager / CommandManager ...     │
│  └─ (自动实例化、Init/Uninit、配置持久化)  │
├─────────────────────────────────────────┤
│         TaskHelper (异步任务层)           │
│  ├─ 多优先级队列 (Channel-based)          │
│  ├─ 同步 / 异步任务统一调度               │
│  ├─ 超时 / 异常 / 取消语义                │
│  └─ Framework 线程安全执行                │
├─────────────────────────────────────────┤
