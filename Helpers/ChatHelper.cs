...
public static unsafe class ChatHelper
{
    ...
    static ChatHelper() =>
        ProcessChatBox ??= ProcessChatBoxSig.GetDelegate<ProcessChatBoxDelegate>();

    public static void SendMessageUnsafe(Utf8String* message)
        => ProcessChatBox(UIModule.Instance(), message, nint.Zero, 0);

    public static void SendMessageUnsafe(ReadOnlySpan<byte> message)
    {
