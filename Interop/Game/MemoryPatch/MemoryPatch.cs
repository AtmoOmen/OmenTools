...
namespace OmenTools.Interop.Game;
...
public class MemoryPatch : IDisposable
{
    ...
    {
        if (ISigScanner.Instance().TryScanModule(sig, out var address))
            return address;

        IPluginLog.Instance().Warning($"未能为 MemoryPatch 搜寻到签名 {sig}");
        return nint.Zero;
    }
    ...
    private static byte?[] ParseByteString(string bytesString) =>
        bytesString.Replace(" ", "")
                   .Chunk(2)
                   .Select
                   (chunk => chunk.SequenceEqual("??") || chunk.SequenceEqual("**")
                                 ? (byte?)null
