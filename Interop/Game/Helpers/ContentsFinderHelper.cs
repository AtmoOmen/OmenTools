...
public static class ContentsFinderHelper
{
    ...
    public static bool RequestDutyNormal(uint rowID, ContentsFinderOption option)
    {
        ...
        DLog.Debug($"[ContentsFinderHelper] 一般副本 ({LuminaWrapper.GetContentName(rowID)})\n{optionFinal}");
    ...
    {
        var optionFinal = option.Clone();
        DLog.Debug($"[ContentsFinderHelper] 一般副本 ({string.Join(", ", rowIDs.Select(LuminaWrapper.GetContentName))})\n{optionFinal}");
    ...
    {
        var optionFinal = option.Clone();
        DLog.Debug($"[ContentsFinderHelper] 一般副本 ({LuminaWrapper.GetContentRouletteName(rowID)})\n{optionFinal}");
    ...
    public static void RequestDutySupport(uint dawnContentID)
    {
        ...
        var partyComposition = new
        {
            ...
            Total = memberType.TanksPerParty   +
                    memberType.HealersPerParty +
                    memberType.MeleesPerParty  +
                    memberType.RangedPerParty
        };
        ...
        foreach (var member in members)
        {
            ...
            var data = member.Data.FirstOrDefault
            (job =>
                {
        ...
        DLog.Debug
        (
            $"[ContentsFinderHelper] 剧情辅助器（{content.Content.Value.Name.ToString()}）\n" +
