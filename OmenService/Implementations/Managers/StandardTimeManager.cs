...
public partial class StandardTimeManager : OmenServiceBase<StandardTimeManager>
{
    ...
    private async Task QueryAll(CancellationToken token)
    {
        if (WebAPIOffset == null)
        {
            ...
            {
                DLog.Error("尝试从 WebAPI 获取标准时间时发生错误", ex);
            }
        }
        ...
        if (Clock == null)
        {
            ...
            {
                DLog.Error("尝试从 ntp.ntsc.ac.cn 获取标准时间时发生错误", ex);
                ...
                {
                    DLog.Error("尝试从 pool.ntp.org 获取标准时间时发生错误", ex);
                }
            }
        }

        DLog.Debug($"[StandardTimeManager] 请求标准时间完成, 当前 UTC 时间: {UTCNow}, 来源类型: {Source}");
    }
