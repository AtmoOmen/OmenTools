...
public unsafe class ContextMenuItemManager : OmenServiceBase<ContextMenuItemManager>
{
    ...
    public Item? GetPrismBoxItem(IMenuOpenedArgs args)
    {
        ...
        return AddonContextMenuEvent.TryScanContextMenuText(LuminaWrapper.GetAddonText(11900), out _) ? LastPrismBoxItem : null;
    }
    ...
    private bool ProcessItem(uint itemID, uint glamourID)
    {
        ...
        if (Sheets.Gears.TryGetValue(itemID, out var gearItem))
        {
            LastItem = gearItem;
            if (glamourID > 0 && Sheets.Gears.TryGetValue(glamourID, out var glamourItem))
    ...
    private bool HandleMiragePrismMiragePlate()
    {
        ...
        if (!Sheets.Gears.TryGetValue(agent->ItemId, out var item)) return false;
        LastItem = item;
    ...
    private bool HandleCabinetWithdraw()
    {
        ...
        if (!Sheets.Gears.TryGetValue(LastDetailItemID, out var item)) return false;
        LastItem = item;
    ...
    {
        if (!Sheets.Gears.TryGetValue(LastHoveredItemID, out var inspectItem)) return false;
        var glamourID = CharacterInspectItems.FirstOrDefault(x => x.ItemId == LastHoveredItemID).GlamourId;

        LastItem        = inspectItem;
        LastGlamourItem = Sheets.Gears.TryGetValue(glamourID, out var gearItem) ? gearItem : LastItem;
