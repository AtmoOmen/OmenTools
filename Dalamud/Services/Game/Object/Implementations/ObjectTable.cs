...
namespace OmenTools.Dalamud.Services.Game.Object;
...
internal sealed partial class ObjectTable : IObjectTable
{
    ...
    public unsafe nint Address =>
        (nint)(&CSGameObjectManager.Instance()->Objects);
    ...
    public IGameObject? this[int index] =>
        index >= ObjectTableLength || index < 0 ?
            null :
            cachedObjectTable[index].Update();
    ...
    public unsafe nint GetObjectAddress(int index) =>
        index >= ObjectTableLength || index < 0 ?
            nint.Zero :
            (nint)cachedObjectTable[index].Address;
    ...
    public unsafe IGameObject? CreateObjectReference(nint address)
    {
        ...
        if (!IPlayerState.Instance().IsLoaded)
            return null;
    ...
    (
        Pointer<CSGameObject>* gameObjectPtr
    )
    {
